...
%---------------------------------------------- basic checks    
    if ~isnumeric(node) || ~isnumeric(edge) || ~isnumeric(vert) || ~isnumeric(fTOL)
        error('inpoly2:incorrectInputClass', 'Incorrect input class.') ;
    end
%---------------------------------------------- basic checks
    if ndims(node) ~= 2 || ndims(edge) ~= 2 || ndims(vert) ~= 2 || numel(fTOL) ~= 1
        error('inpoly2:incorrectDimensions', 'Incorrect input dimensions.');
    end
...
%---------------------------------------------- basic checks
    if (min(edge(:)) < +1 || max(edge(:)) > nnod)
        error('inpoly2:invalidInputs', 'Invalid EDGE input array.') ;
    end
   ...
   [~,ivec] = ...
    ...
    if (exist('OCTAVE_VERSION','builtin')  > +0)
        if (exist('inpoly2_oct','file') == +3)
            %-- delegate to compiled version of the code if it's available
               [stat,bnds] =  inpoly2_oct(vert, node, edge, fTOL, lbar) ;
        else
